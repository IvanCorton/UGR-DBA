...
public class MyFirstTieFighter extends LARVAFirstAgent{
    ...
    final int minBattery = 14, maxBattery = 3486, batteryValue = 5, upValue = 5;
    
    int width, height, maxFlight, initialFlight = 0;
    
    double orientacion = 0; // Empieza mirando a derecha.
    
    double pos[][]=new double[100][];
    String action="UP"; // Para que pueda inicialmente calcular orientacion

    boolean point_at;
    ACLMessage open, session;
    String[] contentTokens,
            mySensors = new String[]{
                "ALIVE", // true/false si esta vivo
                "ONTARGET", // true/false si esta justos por encima de target
                "GPS", // x,y,z
                "COMPASS", // 0,45,90,135, ... // puede manejar con memoria
                "LIDAR", // malla 11 x 11, altura respecto agente, mas alto negativo, agente centor [6][6]
                "ALTITUDE", // altura sobre el nivel del suelo
                "VISUAL", // malla 11 x 11 altura absoluta del suelo
                "ENERGY", // valor de energia
                "PAYLOAD", // numero capturado , solo hay uno en practica 2
                "DISTANCE", // distancia sin direccion
                "THERMAL", // malla 11 x 11, valor de celda mayor si es mas lejo de objetivo
                "ANGULAR"  // angulo del este con el objetivo, agente siempre en principio orientado al este 0
            };
    // Movimientos posibles [RIGHT, LEFT,MOVE,UP,DOWN, RECHARGE,CAPTURE]
    boolean step = true;
    ...
    public Status MySolveProblem() {
        ...
        session=session.createReply();
        session.setContent("Request execute "+"RECHARGE"+" session "+sessionKey);
        this.LARVAsend(session);
        session=this.LARVAblockingReceive();
        if(session.getContent().startsWith("Refuse") || session.getContent().startsWith("Failure")){
            Alert("Reading of sensors failed due to "+session.getContent());
        ...
        }while(!Objects.equals(action, "CAPTURE"));
    ...
    private void cambiar_orientacion() {
